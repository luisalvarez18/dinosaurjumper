...
var tiempoHastaMoneda = 0.5;
var tiempoMonedaMin = 0.7;
var tiempoMonedaMax = 2.7;
var maxMonedaY = 240;
var minMonedaY = 100;
var monedas = [];
var velMoneda = 0.5;
...
function CrearMoneda() {
    ...
    moneda.style.bottom = minMonedaY + Math.random() * (maxMonedaY-minMonedaY)+"px";
...
function MoverMonedas() {
    for (var i = monedas.length - 1; i >= 0; i--) {
        if(monedas[i].posX < -monedas[i].clientWidth) {
            ...
            monedas[i].posX -= CalcularDesplazamiento() * velMoneda;
...
function DetectarColisionObstaculo() {
...
function DetectarColisionMoneda() {
    for (var i = 0; i < monedas.length; i++) {
        if(monedas[i].posX > dinoPosX + dino.clientWidth) {
            //EVADE
            break; //al estar en orden, no puede chocar con más
        }else{
            if(IsCollision(dino, monedas[i], 10, 30, 15, 20)) {
                score++;
                textoScore.innerText = score;
                monedas[i].classList.add("moneda-oculta");
            }
        }
    }
}
